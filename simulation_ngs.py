import mesa
import random
import matplotlib.pyplot as plt
import pandas as pd
import numpy as np

# --- 1. Agenten ---

class PassengerAgent(mesa.Agent):
    def __init__(self, unique_id, model, transport_type_preference):
        super().__init__(model)
        self.unique_id = unique_id
        self.transport_type = transport_type_preference 
        self.waiting_time = 0
        self.satisfaction = 100
        self.arrived = False
        self.has_switched = False 

    def step(self):
        if not self.arrived:
            self.waiting_time += 1
            
            # Zufriedenheit sinkt (Soziale Dimension)
            decay = 0.5 if self.transport_type == "Tram" else 0.8
            self.satisfaction -= decay
            if self.satisfaction < 0: self.satisfaction = 0

            # --- Verhalten 2. Ordnung: Mode-Switching ---
            # Wenn Bus-Wartezeit zu lang, wechselt der Agent zum Tram
            if self.transport_type == "Bus" and self.waiting_time > 25:
                self.switch_to_tram()

    def switch_to_tram(self):
        self.transport_type = "Tram"
        self.has_switched = True
        self.satisfaction -= 15 # Wechselstress
        # Agent geht zur Tram-Haltestelle (y=1)
        x, y = self.pos
        if y == 0: 
            self.model.grid.move_agent(self, (x, 1))

class VehicleAgent(mesa.Agent):
    def __init__(self, unique_id, model, v_type):
        super().__init__(model)
        self.unique_id = unique_id
        self.v_type = v_type 
        self.emissions = 0 # Ökologische Dimension (CO2)

    def step(self):
        x, y = self.pos
        moved = True
        
        # Baustelle bremst nur Busse
        if self.v_type == "Bus" and self.model.construction_active:
            if 10 <= x <= 20:
                if random.random() < 0.8: # Hohe Stauwahrscheinlichkeit
                    moved = False
                    # CO2: Stehendes Fahrzeug im Stau (Stop & Go)
                    self.emissions += 8 
        
        if moved:
            new_x = (x + 1) % self.model.grid.width 
            self.model.grid.move_agent(self, (new_x, y))
            
            # CO2: Fahren
            if self.v_type == "Bus":
                self.emissions += 5 # Dieselbus
            else:
                self.emissions += 1 # Tram (Strommix, effizienter)
        
        # Passagiere einsammeln
        if moved:
            cell_mates = self.model.grid.get_cell_list_contents([(new_x, y)])
            capacity = 0
            for mate in cell_mates:
                if isinstance(mate, PassengerAgent) and not mate.arrived:
                    if mate.transport_type == self.v_type:
                        # Einfache Kapazitätsgrenze (optional, hier unbegrenzt)
                        mate.arrived = True 

# --- 2. Modell ---

class CityTransportModel(mesa.Model):
    def __init__(self, num_passengers, scenario="baseline"):
        super().__init__()
        self.scenario = scenario
        self.construction_active = True # Immer Baustelle
        self.grid = mesa.space.MultiGrid(50, 2, torus=True)
        
        self.datacollector = mesa.DataCollector(
            model_reporters={
                "Avg_Satisfaction": compute_avg_satisfaction,
                "Total_Switchers": count_switchers,
                # Summe aller Emissionen im System
                "Total_CO2": lambda m: sum([a.emissions for a in m.agents if isinstance(a, VehicleAgent)])
            }
        )

        # FAHRZEUGE INITIALISIEREN
        # ID-Counter
        agent_id = 0
        
        # Busse: Immer gleich (Linie ist betroffen)
        bus = VehicleAgent(agent_id, self, "Bus")
        self.grid.place_agent(bus, (0, 0))
        agent_id += 1

        # Trams: Hier unterscheidet sich die "Lösung" vom "Status Quo"
        # Im Szenario "solution" verdichten wir den Takt (mehr Trams)
        num_trams = 3 if scenario == "solution" else 1
        
        for i in range(num_trams):
            tram = VehicleAgent(agent_id, self, "Tram")
            # Verteile Trams gleichmäßig
            start_x = int((50 / num_trams) * i)
            self.grid.place_agent(tram, (start_x, 1))
            agent_id += 1

        # Passagiere
        for i in range(num_passengers):
            p_type = "Bus" if random.random() < 0.7 else "Tram"
            a = PassengerAgent(agent_id, self, p_type)
            agent_id += 1
            x = random.randrange(self.grid.width)
            y = 0 if p_type == "Bus" else 1
            self.grid.place_agent(a, (x, y))

    def step(self):
        self.datacollector.collect(self)
        self.agents.shuffle_do("step")

# --- Hilfsfunktionen ---

def compute_avg_satisfaction(model):
    passengers = [a for a in model.agents if isinstance(a, PassengerAgent)]
    if not passengers: return 0
    return sum([a.satisfaction for a in passengers]) / len(passengers)

def count_switchers(model):
    return sum([1 for a in model.agents if isinstance(a, PassengerAgent) and a.has_switched])

# --- 3. Vergleichs-Ausführung (Batch Run manuell) ---

if __name__ == "__main__":
    steps = 150
    n_passengers = 200
    
    # --- LAUF 1: Baseline (Status Quo) ---
    model_base = CityTransportModel(n_passengers, scenario="baseline")
    for i in range(steps): model_base.step()
    res_base = model_base.datacollector.get_model_vars_dataframe()

    # --- LAUF 2: Solution (Taktverdichtung) ---
    model_sol = CityTransportModel(n_passengers, scenario="solution")
    for i in range(steps): model_sol.step()
    res_sol = model_sol.datacollector.get_model_vars_dataframe()

    # --- PLOTTING FÜR NGS ---
    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(10, 12), sharex=True)

    # 1. Soziale Dimension: Zufriedenheit
    ax1.plot(res_base["Avg_Satisfaction"], label="Status Quo (1 Tram)", color="red", linestyle="--")
    ax1.plot(res_sol["Avg_Satisfaction"], label="Lösung (3 Trams)", color="green")
    ax1.set_title("Soziale Dimension: Fahrgastzufriedenheit")
    ax1.set_ylabel("Score (0-100)")
    ax1.legend()
    ax1.grid(True)

    # 2. Ökologische Dimension: CO2-Emissionen (Kumuliert)
    # Hinweis: Lösung hat mehr Trams (Emissionen steigen leicht), aber Busse stauen weniger? 
    # Oder: Wir sehen, ob die Gesamtemissionen gerechtfertigt sind.
    ax2.plot(res_base["Total_CO2"], label="Status Quo CO2", color="grey", linestyle="--")
    ax2.plot(res_sol["Total_CO2"], label="Lösung CO2", color="green")
    ax2.set_title("Ökologische Dimension: CO2 Emissionen (Kumuliert)")
    ax2.set_ylabel("Emissionseinheiten")
    ax2.legend()
    ax2.grid(True)

    # 3. Verhalten: Umsteiger (Stressindikator)
    ax3.plot(res_base["Total_Switchers"], label="Umsteiger (Status Quo)", color="red", linestyle="--")
    ax3.plot(res_sol["Total_Switchers"], label="Umsteiger (Lösung)", color="green")
    ax3.set_title("Verhalten (2nd Order Effect): Erzwungenes Umsteigen")
    ax3.set_xlabel("Zeit (Ticks)")
    ax3.set_ylabel("Anzahl Personen")
    ax3.legend()
    ax3.grid(True)

    plt.tight_layout()
    plt.show()