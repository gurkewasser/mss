...
# --- KONFIGURATION: REALITÄT AB BILDEN ---
...
class PassengerAgent(mesa.Agent):
    ...
    def step(self):
        if not self.arrived:
            ...
            # Alle 20 Sekunden sinkt die Zufriedenheit leicht
            self.satisfaction -= (decay_factor * 0.5) 
            if self.satisfaction < 0: self.satisfaction = 0

            # --- KIPP-PUNKT (Switching Logic) ---
            # Wenn man länger als 5 Minuten wartet (Realzeit), wechselt man
    ...
    def switch_to_tram(self):
        ...
        self.satisfaction -= 15 # Ärger über das Wechseln
...
class VehicleAgent(mesa.Agent):
    ...
    def __init__(self, unique_id, model, v_type):
        ...
        # 20 km/h = 5.5 m/s = ~110m pro 20sek = 11 Zellen
        if v_type == "Tram":
            self.speed = 4 # Tram ist konstant (etwas langsamer simuliert für stops)
        else:
    ...
    def step(self):
        ...
        # Da wir diskrete Sprünge machen, schauen wir uns den Bereich an, den wir überfahren haben
    ...
    def __init__(self, num_passengers, construction_active=True):
        ...
        self.datacollector = mesa.DataCollector(
            model_reporters={
                ...
                "Switchers": count_switchers
            }
        )

        # 1. Fahrzeuge erstellen
        # Wir setzen 3 Busse und 3 Trams ein (dichter Takt)
        for i in range(3):
...
if __name__ == "__main__":
    # Simulation: 30 Minuten Pendlerverkehr (ca. 90 Ticks)
    ...
    ax1b = ax1.twinx()
    ax1b.plot(results["Time"], results["Switchers"], label="Umsteiger (zur Tram)", color="red", linestyle="--")
